...
# --- Globals ---
WORD_VECTORS: Dict[str, np.ndarray] = {}; WORD_BUCKETS: Dict[str, List[str]] = {}
LEMMA_MAP: Dict[str, str] = {}; COMMON_WORDS_TO_SKIP: Set[str] = set()
ANTONYM_MAP: Dict[str, List[str]] = {}; ANNOY_INDEX: AnnoyIndex = None
ID_TO_WORD: Dict[int, str] = {}; VECTOR_DIMENSIONS = 768

# =====================================================================
# FINAL GAMEPLAY TUNING CONSTANTS - The "Feel" of the Game
# =====================================================================
# Raw similarity must be above this to score any points.
SIMILARITY_FLOOR = 0.6
# Raw similarity at or above this is considered a "great" guess.
SIMILARITY_CEILING = 0.87
# A power < 1.0 makes the score increase quickly at first. This makes
# medium-quality guesses feel more rewarding.
PROGRESS_CURVE_POWER = 0.5
# A fixed, low score for antonyms.
ANTONYM_PROGRESS_SCORE = 70
# Ideal raw similarity range for a hint.
HINT_IDEAL_MIN, HINT_IDEAL_MAX = 0.75, 0.87
...
def load_precomputed_data():
    ...
    WORD_VECTORS = joblib.load(paths["vectors"]); LEMMA_MAP = joblib.load(paths["lemmas"]); ANTONYM_MAP = joblib.load(paths["antonyms"])
    ...
    ANNOY_INDEX = AnnoyIndex(VECTOR_DIMENSIONS, 'angular'); ANNOY_INDEX.load(paths["annoy_index"])
...
# ... (lemmatize, similarity_score, and get_progress_score definitions are unchanged) ...
...
# ... (All endpoints [/, /similarity, /hint, /random-word] are unchanged) ...
@app.get("/similarity")
...
class HintRequest(BaseModel): word: str; exclude: List[str] = []
@app.post("/hint")
def get_hint(data: HintRequest):
    target_word_lemma = lemmatize(data.word)
    if target_word_lemma not in WORD_VECTORS: return {"hint": None, "reason": "word not in model"}
    exclude_lemmas = {lemmatize(w) for w in data.exclude}; exclude_lemmas.add(target_word_lemma)
    ...
    for item_id in neighbor_ids:
        ...
        if hint_word and hint_word not in exclude_lemmas:
            ...
            if HINT_IDEAL_MIN <= raw_sim <= HINT_IDEAL_MAX: ideal_candidates.append(hint_data)
